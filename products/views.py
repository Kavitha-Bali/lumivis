...
def serve_static(request, path):
    ...
    Cached aggressively, same reasoning as serve_media: these paths are already
    cache-busted with a `?v=` query string in the templates, so it's safe to tell
    browsers to keep the response for a year.
    """
    try:
